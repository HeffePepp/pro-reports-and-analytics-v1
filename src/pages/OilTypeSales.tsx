...
const OilTypeSalesPage: React.FC = () => {
  ...
  const maxRev = useMemo(
    () => Math.max(...oilMixRows.map((r) => r.revenue), 1),
    []
  );

  const regenerateInsights = () => {
    const best = oilMixRows.reduce((best, r) =>
      !best || r.avgTicket > best.avgTicket ? r : best
    );
    setInsights([
      `"${best.oilType}" has the highest average ticket at $${best.avgTicket.toFixed(
        0
      )}.`,
    ...
    <ShellLayout
      ...
      rightInfo={
        <>
          <span>
            Period:{" "}
            <span className="font-medium">{oilMixSummary.periodLabel}</span>
          </span>
        </>
      }
    >
      ...
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Volume and revenue mix across conventional, blend, synthetic and
            high mileage oils.
          </p>
        </div>
      </div>

      {/* Layout: left content + right AI panel */}
      <div className="mt-4 grid grid-cols-1 lg:grid-cols-4 gap-4">
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-3">
            <MetricTile
              label="Oil change ROs"
              value={oilMixSummary.totalOilROs.toLocaleString()}
            />
            <MetricTile
              label="Oil revenue"
              value={`$${oilMixSummary.totalRevenue.toLocaleString()}`}
            />
            <MetricTile
              label="Synthetic share"
              value={`${oilMixSummary.syntheticSharePct.toFixed(0)}%`}
            />
            <MetricTile
              label="High mileage share"
              value={`${oilMixSummary.highMileageSharePct.toFixed(0)}%`}
            />
            <MetricTile
              label="Avg ticket (all)"
              value={`$${(
                oilMixSummary.totalRevenue / oilMixSummary.totalOilROs
              ).toFixed(0)}`}
            />
          </div>
          ...
          <div className="block lg:hidden">
            <AIInsightsTile
              title="AI Insights"
              subtitle="Based on oil mix & revenue"
              bullets={insights}
              ...
              {oilMixRows.map((r) => (
                <div key={r.oilType}>
                  <div className="flex justify-between text-[11px]">
                    <span>{r.oilType}</span>
                    <span>
                      ${r.revenue.toLocaleString()} · {r.roCount.toLocaleString()}{" "}
                      ROs
                    </span>
                  </div>
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-[10px] text-slate-500 w-32 text-right">
                      Avg ticket ${r.avgTicket.toFixed(0)}
                    </span>
                  </div>
                </div>
              ))}
          ...
          <section className="rounded-2xl bg-white border border-slate-200 shadow-sm p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-[11px] text-slate-500">
                Volume and average ticket by oil type
              </span>
            </div>
