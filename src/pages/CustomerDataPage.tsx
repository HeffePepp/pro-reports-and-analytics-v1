...
// Dummy summary and rows – plug Airtable/API later
...
const customerRows: CustomerRow[] = [
  {
    name: "Jane Smith",
    vehicle: "2018 Toyota Camry · 7XYZ123",
    visits: 5,
    ltv: 642,
    ticketAvg: 128,
  ...
  },
  {
    ...
    vehicle: "2016 Ford F-150 · 3ABC456",
    visits: 3,
    ltv: 297,
    ticketAvg: 99,
  ...
  {
    name: "Laura Chen",
    vehicle: "2021 Subaru Outback · 9LMN789",
    visits: 2,
    ltv: 210,
    ticketAvg: 105,
  ...
  {
    name: "Carlos Garcia",
    vehicle: "2012 Honda Civic · 4DEF321",
    visits: 1,
    ltv: 79,
    ticketAvg: 79,
...
const CustomerDataPage: React.FC = () => {
  ...
  const highValueCount = useMemo(
    () => customerRows.filter((c) => c.ltv >= 300).length,
    []
  );
      ...
      <div className="flex-1 flex flex-col">
        ...
        <header className="h-14 bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-6">
          ...
          <div className="flex items-center gap-3 text-xs text-slate-500">
            <span>
              Store: <span className="font-medium">{customerSummary.storeName}</span>
            </span>
            <span>
              Period:{" "}
              <span className="font-medium">{customerSummary.periodLabel}</span>
            </span>
          </div>
        </header>
        ...
        <main className="flex-1 overflow-y-auto px-4 md:px-6 py-4 md:py-6 space-y-6">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Customer and vehicle roster with visit history, lifetime value and
                contactability across channels.
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
            <CustomerTile
              ...
              value={customerSummary.avgVisits.toFixed(1)}
            />
            <CustomerTile
              label="Avg lifetime value"
              value={`$${customerSummary.avgLtv.toFixed(0)}`}
            />
            <CustomerTile
              label="Reachable by email"
              value={`${customerSummary.emailReachPct}%`}
            />
            <CustomerTile
              label="Reachable by SMS"
              value={`${customerSummary.smsReachPct}%`}
            />
            <CustomerTile
              label="Reachable by mail"
              value={`${customerSummary.mailReachPct}%`}
            />
          </div>
          ...
          <section className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="rounded-2xl bg-white border border-slate-200 shadow-sm p-4 flex flex-col">
              ...
              <p className="mt-3 text-[11px] text-slate-400">
                ...
                customer metrics to generate store-specific recommendations and
                segments.
              </p>
            </div>
            ...
            <div className="lg:col-span-2 rounded-2xl bg-white border border-slate-200 shadow-sm p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-[11px] text-slate-400">
                  Contactability and value (dummy data)
                </span>
              </div>
                  ...
                  <tbody>
                    {customerRows.map((row) => (
                      <tr key={row.name + row.vehicle} className="border-t border-slate-100">
                        ...
                        <td className="py-2 pr-3 text-right">
                          ${row.ticketAvg.toFixed(0)}
                        </td>
                        <td className="py-2 pr-3 text-slate-600">{row.lastVisit}</td>
                        <td className="py-2 pr-3">
                          <ContactBadge type="email" status={row.emailStatus} />
                        </td>
                        <td className="py-2 pr-3">
                          <ContactBadge type="sms" status={row.smsStatus} />
                        </td>
                        <td className="py-2 pr-3">
                          <ContactBadge type="mail" status={row.mailStatus} />
                        </td>
                      </tr>
                    ))}
                  </tbody>
...
const CustomerTile: React.FC<{ label: string; value: string }> = ({
  label,
  value,
}) => (
...
const Badge: React.FC<{ color: string; label: string }> = ({ color, label }) => (
  <span
    className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] border border-slate-100 ${color}`}
  >
    {label}
  </span>
);
