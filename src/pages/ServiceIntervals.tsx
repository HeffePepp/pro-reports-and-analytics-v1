...
type ServiceIntervalBucket = {
  label: string;
  daysMin: number;
  daysMax: number | null; // null = open-ended
  customers: number;
  vehicles: number;
  avgTicket: number;
};
...
const ServiceIntervalsPage: React.FC = () => {
  ...
  const currentPct = useMemo(
    () => (siSummary.currentCount / siSummary.totalCustomers) * 100,
    []
  );
  const atRiskPct = useMemo(
    () => (siSummary.atRiskCount / siSummary.totalCustomers) * 100,
    []
  );
  const lostPct = useMemo(
    () => (siSummary.lostCount / siSummary.totalCustomers) * 100,
    []
  );
  const maxBucketCustomers = useMemo(
    () => Math.max(...siBuckets.map((b) => b.customers), 1),
    []
  );
  ...
  const regenerateInsights = () => {
    ...
    setInsights([
      `${currentPct.toFixed(
        1
      )}% of customers are current, with ${atRiskPct.toFixed(
        1
      )}% at-risk and ${lostPct.toFixed(1)}% lost.`,
        ...
        <>
          <span>
            ...
            <span className="font-medium">
              {siSummary.totalCustomers.toLocaleString()}
            </span>
          </span>
        </>
      }
      ...
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Snapshot of current, at-risk and lost customers by time since last
            visit and mileage interval.
          </p>
        </div>
      </div>
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-3">
            <MetricTile
              ...
              value={siSummary.totalCustomers.toLocaleString()}
            />
            <MetricTile
              label="Current"
            ...
            <MetricTile
              ...
              value={siSummary.avgDaysSinceVisit.toFixed(0)}
            />
          </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-sm font-semibold text-slate-900">
                  Customers by service interval
                </h2>
                <p className="text-[11px] text-slate-600">
                  Buckets by time since last visit (dummy data)
                </p>
              </div>
            </div>
              ...
              {siBuckets.map((b) => (
                <div key={b.label}>
                  <div className="flex justify-between text-[11px]">
                    <span>{b.label}</span>
                    <span>
                      {b.customers.toLocaleString()} customers ·{" "}
                      {b.vehicles.toLocaleString()} vehicles
                    </span>
                  </div>
                  <div className="flex items-center gap-2 mt-1">
                    <div className="flex-1 h-2 rounded-full bg-slate-100 overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${
                            (b.customers / maxBucketCustomers) * 100
                          }%`,
                        }}
                      />
                    </div>
                    <span className="text-[10px] text-slate-500 w-32 text-right">
                      Avg ticket ${b.avgTicket.toFixed(0)}
                    </span>
                  </div>
                </div>
              ))}
